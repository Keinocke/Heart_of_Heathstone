...
class AT_007:
    """Spellslinger - simplified vanilla/no-op.

    Original gives random spells and applies AT_007e cost reduction.
    Simplified because AT_007e/random spell generation can break broad simulation.
    """
    pass
...
class BAR_079t10:
    ...
    play = Buff(FRIENDLY_MINIONS - SELF, "EX1_019e")


class BAR_079t10b:
    """Wildvine\nBattlecry: Give your other minions +2/+2.\n"""
    play = Buff(FRIENDLY_MINIONS - SELF, "AT_117e")
...
class BAR_534:
    ...
    play = Buff(FRIENDLY_MINIONS, "EX1_195e")
...
class BRM_024:
    """Drakonid Crusher - simplified: vanilla minion.

    Original: Battlecry: If your opponent has 15 or less Health, gain +3/+3.
    Simplified because the generated conditional stat buff can crash aura refresh.
    """
    pass
...
class CFM_614:
    ...
    play = Buff(FRIENDLY_MINIONS, "EX1_019e")
...
class EDR_209a:
    ...
    play = Buff(FRIENDLY_MINIONS - SELF, "EX1_195e")
...
class OG_195b:
    ...
    play = Buff(FRIENDLY_MINIONS, "AT_117e")
...
class OG_256:
    ...
    deathrattle = Buff(FRIENDLY_MINIONS, "EX1_019e")
...
class CORE_REV_351:
    """Roosting Gargoyle - disabled auto script for stability."""
    play = ()


class VAN_EX1_246:
    """Hex - transform disabled for stability."""
    play = ()
...
class TID_708:
    """Polymorph: Jellyfish - transform disabled for stability."""
    play = ()


class DINO_419:
    """Herbivore Assistant - disabled auto script for stability."""
    play = ()
...
class VAN_CS2_022:
    """Polymorph - transform disabled for stability."""
    play = ()
...
def _is_risky_auto_text(text: str) -> bool:
    """Return True for card text that should NOT receive generated auto-logic.

    These cards should still load and be playable, but as vanilla/no-op cards
    unless we add a manual safe override later.
    """
    lower = _clean_card_text(text).lower()

    risky_phrases = (
        # Choice/random generation often creates illegal cards/targets.
        "discover",
        "random",
        "from another class",
        "outside your class",
        ...
        # Transform/evolve effects have caused delayed_destruction/None issues.
        "transform",
        "evolve",

        # Trigger/event text often creates bad aura/stat refresh behavior.
        "whenever",
        "after you",
        "after this",
        "after your",
        ...
        # Keyword application can accidentally target heroes.
        "give a minion poisonous",
        "give a minion lifesteal",
        "give a minion windfury",
        "give a friendly minion poisonous",
        "give a friendly minion lifesteal",
        "give a friendly minion windfury",
        "immune this turn",

        # Mechanics we are not ready to auto-generate safely.
        "secret",
        "quest",
        "titan",
        "colossal",
    ...
    return any(phrase in lower for phrase in risky_phrases)
...
def _make_placeholder_class(card_id: str, entity):
    ...
    card_type = str(_get_value(entity, "CARDTYPE") or "")
    risky_auto_text = _is_risky_auto_text(text)

    # Static keyword tags are only safe on minions/weapons.
    # Avoid putting Lifesteal/Windfury/Poisonous/etc. onto heroes/spells.
    if card_type in ("4", "7"):  # 4=minion, 7=weapon
        auto_tags = _auto_static_tags(text)
        if auto_tags is not None:
            existing_tags = metadata.get("tags", {})
            merged_tags = dict(existing_tags)
            merged_tags.update(auto_tags)
            metadata["tags"] = _safe_numeric_tags(merged_tags)

    # If text is complex/risky, keep the card loadable/playable but do not
    # attach generated effects. Manual overrides below can still implement it.
    if not risky_auto_text:
    ...
    # Discover/Choose/Inspire are deliberately NOT auto-generated here.
    # They are common sources of random illegal target/card paths.
        ...
        if card_id in globals():
            ...
            if card_type in ("4", "7"):  # 4=minion, 7=weapon
                ...
                if auto_tags is not None:
                    ...
                    merged_tags = dict(existing_tags)
                    merged_tags.update(auto_tags)
                    cls.tags = _safe_numeric_tags(merged_tags)

            # Only add generated gameplay logic to existing classes if the text is simple.
            # Risky cards remain vanilla/no-op unless manually implemented below.
            if not has_logic and not risky_auto_text:
                auto_play = _auto_battlecry_play(card_id, text)
                if auto_play is not None:
                    cls.play = auto_play
...
class AT_081:
    """Eadric the Pure - simplified no-op.

    Original: Battlecry: Change all enemy minions' Attack to 1.
    Simplified because generated stat-setting logic can corrupt aura/stat refresh.
    """
    pass
...
class AT_108:
    ...
    Always gains Charge.
    """
    play = SetTags(SELF, (GameTag.CHARGE,))
...
def _main_text_only(text: str) -> str:
    """
    Remove nested mechanic text so generic play parsers do not read
    Deathrattle/Inspire/Combo text as normal play text.
    """
...
def _set_keyword(selector, *tags):
    return SetTags(selector, tuple(tags))
...
def _auto_battlecry_requirements(text: str):
    ...
    needs_target = (
        ...
        or "give an enemy minion" in lower
        or "give a beast" in lower
        or "give a friendly beast" in lower
        or "give a mech" in lower
        or "give a friendly mech" in lower
    )
    ...
    if any(phrase in lower for phrase in minion_target_phrases):
        req[PlayReq.REQ_MINION_TARGET] = 0

    if "friendly" in lower:
        req[PlayReq.REQ_FRIENDLY_TARGET] = 0

    if "enemy" in lower:
        req[PlayReq.REQ_ENEMY_TARGET] = 0

    return req

SAFE_BUFFS = {
    # Attack-only approximations
    "PLUS_1_ATTACK": "AT_029e",
    "PLUS_2_ATTACK": "CS2_073e",
    "PLUS_3_ATTACK": "CS2_073e",
    "PLUS_4_ATTACK": "CS2_073e",

    # Health-only approximations
    "PLUS_1_HEALTH": "EX1_195e",
    "PLUS_2_HEALTH": "EX1_195e",
    "PLUS_3_HEALTH": "EX1_195e",

    # Attack/health
    "PLUS_1_PLUS_1": "EX1_019e",
    "PLUS_2_PLUS_2": "AT_117e",
    "PLUS_3_PLUS_3": "BAR_313e",
    "PLUS_4_PLUS_4": "DRG_650e",

    # Cost reductions
    "COST_MINUS_1": "AT_014e",
    "COST_MINUS_2": "AT_014e",
    "COST_MINUS_3": "AT_014e",

    # Safe keyword fallbacks if SetTags is unsuitable
    "SAFE_KEYWORD_FALLBACK": "EX1_019e",
}


def _safe_buff(name: str) -> str:
    return Buff(TARGET, _safe_buff("PLUS_3_HEALTH"))
...
def _auto_battlecry_play(card_id: str, text: str):
    ...
    if "give a minion +2 attack" in lower:
        return Buff(TARGET, "CS2_073e")

    if "give a friendly minion +2 health" in lower:
        return Buff(TARGET, "EX1_195e")
    ...
    if "give a friendly beast +3/+3" in lower:
        return Buff(TARGET, "BAR_313e")

    if "give a friendly mech +1/+1" in lower:
        return Buff(TARGET, "AT_032e")

    if "give your other murlocs +1/+1" in lower:
        return Buff(FRIENDLY_MINIONS + MURLOC, "AT_032e")

    if "give your other murlocs +2 health" in lower:
        return Buff(FRIENDLY_MINIONS + MURLOC, "EX1_195e")

    if "give all minions in your hand +1/+1" in lower:
        return Buff(FRIENDLY_HAND + MINION, "AT_032e")

    if "give all minions in your hand +2/+2" in lower:
        return Buff(FRIENDLY_HAND + MINION, "AT_117e")

    # -----------------------------------------------------------------------
    # More common self buffs
    # -----------------------------------------------------------------------
    if "gain +1/+1" in lower:
        return Buff(SELF, "AT_032e")

    if "gain +2/+2" in lower:
        return Buff(SELF, "AT_117e")

    if "gain +3/+3" in lower:
        return Buff(SELF, "BAR_313e")

    if "gain +4/+4" in lower:
        return Buff(SELF, "DRG_650e")

    if "gain taunt and divine shield" in lower:
        return _set_keyword(SELF, GameTag.TAUNT, GameTag.DIVINE_SHIELD)

    if "gain stealth" in lower:
        return _set_keyword(SELF, GameTag.STEALTH)

    if "gain rush" in lower:
        return _set_keyword(SELF, GameTag.RUSH)
...
def _auto_deathrattle_play(card_id: str, text: str):
    ...
    if "give your minions +1/+1" in lower:
        return Buff(FRIENDLY_MINIONS, "AT_032e")
...
def _auto_spell_play(card_id: str, text: str):
    ...
    if "gain 8 armor" in lower:
        return GainArmor(FRIENDLY_HERO, 8)
    ...
    if "transform your minions into random legendary minions" in lower:
        return Buff(FRIENDLY_MINIONS, "BAR_313e")
    return None
...
def _auto_inspire_play(card_id: str, text: str):
    ...
    if "give your hero +2 attack" in lower:
        return Buff(FRIENDLY_HERO, "CS2_073e")

    if "deal 2 damage to the enemy hero" in lower:
        return Hit(ENEMY_HERO, 2)

    if "give your other minions +1/+1" in lower:
        return Buff(FRIENDLY_MINIONS - SELF, "AT_032e")
    ...
    if "can attack as normal this turn" in lower:
        return _set_keyword(SELF, GameTag.CHARGE)
    ...
    if "give your totems +2 attack" in lower:
        return Buff(FRIENDLY_MINIONS + TOTEM, "CS2_073e")
...
def _safe_numeric_tags(tags):
    """Keep only safe numeric/static GameTag values.
    ...
    for tag, value in tags.items():
        if tag in numeric_tags:
            if isinstance(value, int):
                ...
                # Drop unsafe dynamic SET(), lambda, action, selector, etc.
                continue
...
# ---------------------------------------------------------------------------
# Safety pass: strip risky auto/event logic from complex cards
# ---------------------------------------------------------------------------

def _sanitize_risky_card_scripts():
    """Remove unstable generated logic from risky cards.

    This keeps cards loadable and playable as vanilla/no-op cards unless a manual
    safe override below re-adds a stable effect.
    """
    risky_attrs = (
        "events",
        "deathrattle",
    ...
    try:
        for entity in _iter_carddefs_entities():
            card_id = _get_card_id(entity)
            text = _get_text(entity, "CARDTEXT")
            if card_id and _is_risky_auto_text(text):
                risky_ids.add(card_id)
    except Exception:
        return

    # Extra known crash-prone families from stress tests.
    risky_ids.update({
        "SCH_605",      # Lake Thresher
        "SW_077",       # Stockades Prisoner
        "RLK_955",      # Silvermoon Armorer
        "SW_076",       # City Architect
        "GVG_107",      # Enhance-o Mechano
        "DRG_050",      # Devoted Maniac
        "VAC_432",      # Resort Valet
        "DAL_563",      # Eager Underling
        "VAC_945",      # Party Planner Vona
        "YOP_009",      # Rally!
        "VAN_EX1_399",  # Gurubashi Berserker
    })

    for card_id in risky_ids:
        cls = globals().get(card_id)
        if cls is None:
            continue
        ...
        # Remove play only for very risky text cards. Simple Hit/Heal manual cards
        # should usually keep play, but random/discover/aura/deathrattle cards should not.
        try:
            entity = None
            for e in _iter_carddefs_entities():
                if _get_card_id(e) == card_id:
                    entity = e
                    break

            text = _get_text(entity, "CARDTEXT") if entity is not None else ""
            lower = _clean_card_text(text).lower()
            ...
            if any(p in lower for p in remove_play_phrases):
                for attr in risky_play_attrs:
                    if hasattr(cls, attr):
                        try:
                            delattr(cls, attr)
                        except Exception:
...
_sanitize_risky_card_scripts()

# ---------------------------------------------------------------------------
# Manual safe overrides for unstable auto-generated modern cards
# ---------------------------------------------------------------------------

class REV_240:
    """Tome Tampering - simplified no-op.

    Original effect involves hand/deck manipulation.
    Simplified because the auto-generated version can crash during is_playable.
    """
    pass


class LOE_029:
    """Jeweled Scarab - simplified no-op.

    Original: Discover a 3-Cost card.
    Discover/random generation will be implemented later using a safe pool.
    """
    pass


class TOY_006:
    """Scarab Keychain - simplified no-op.

    Original has Discover-like behavior.
    Simplified because generated effects can try to apply illegal lifesteal to a Hero.
    """
    pass


class CORE_LOE_076:
    """Sir Finley Mrrgglton - simplified no-op.

    Original changes hero power.
    Simplified because generated choice/keyword logic can crash.
    """
    pass


class VAN_CS2_221:
    """Spiteful Smith - simplified vanilla minion.

    Original: Enrage: Your weapon has +2 Attack.
    Weapons are disabled for now, and generated aura/stat logic can crash.
    """
    pass
...
class DAL_554:
    """Chef Nomi - simplified vanilla minion.

    Original: If your deck is empty, summon six 6/6 Greasefire Elementals.
    Simplified first to avoid generated board/aura crash paths.
    """
    pass
# ---------------------------------------------------------------------------
# Manual safe overrides: cards whose enchantment entities are missing
# ---------------------------------------------------------------------------

class AT_054:
    """The Mistcaller - simplified no-op.

    Original: Give all minions in your hand and deck +1/+1.
    Simplified because AT_054e is not present in CardDefs_new.xml.
    """
    pass


class AT_057:
    """Stablemaster - simplified no-op.

    Original: Give a Beast Immune this turn.
    Simplified because AT_057e is not present in CardDefs_new.xml.
    """
    pass

class GIL_513:
    """Lost Spirit - simplified vanilla minion.

    Original: Deathrattle: Give your minions +1 Attack.
    Simplified because generated deathrattle/stat logic can crash is_playable
    with TypeError: unsupported operand type(s) for +=: 'int' and 'function'.
    """
    pass  
class BT_008:
    """Rustsworn Initiate - simplified vanilla minion.

    Simplified because generated stat/aura logic can crash is_playable with:
    TypeError: unsupported operand type(s) for +=: 'int' and 'function'.
    """
    pass

class AT_084:
    """Lance Carrier - simplified.

    Original: Battlecry: Give a friendly minion +2 Attack this turn.
    Simplified: Give a friendly minion +2 Attack using a stable DB-backed buff.
    """
    requirements = {
        PlayReq.REQ_TARGET_TO_PLAY: 0,
        PlayReq.REQ_MINION_TARGET: 0,
...
# ---------------------------------------------------------------------------
# Manual safe overrides: rare is_playable/stat crashes
# ---------------------------------------------------------------------------

class EX1_020:
    """Scarlet Crusader - simplified vanilla minion.

    Original has Divine Shield. Simplified because generated keyword/stat logic
    can trigger TypeError during is_playable in random simulations.
    """
    pass


class DMF_004:
    """Yogg-Saron, Master of Fate - simplified vanilla minion.

    Original has complex random spell/wheel behavior.
    Simplified to avoid generated random/aura crashes.
    """
    pass


class WON_142:
    """Menagerie Jug - simplified vanilla minion.

    Original buffs minion types in hand/board.
    Simplified to avoid generated stat modifier crashes.
    """
    pass


class CORE_LOOT_013:
    """Vulgar Homunculus - simplified vanilla minion.

    Original Battlecry deals damage to your hero.
    Simplified to avoid generated stat/keyword crash paths.
    """
    pass


class OG_247:
    """Twisted Worgen - simplified vanilla minion.

    Original has Stealth.
    Simplified to avoid generated keyword/stat crash paths.
    """
    pass


class WON_331:
    """Grimestreet Informant - simplified no-op.

    Original Discover effect can generate illegal Hero keyword assignment.
    """
    pass


class VAC_304:
    """Tidepool Pupil - simplified vanilla minion.

    Original has spell-school/copy style behavior.
    Simplified to avoid generated stat modifier crash during is_playable.
    """
    pass

class TOY_037:
    """Hidden Objects - simplified no-op.

    Original has Discover/random-generation behavior.
    Simplified because generated effect can try to apply poisonous to a Hero.
    """
    pass

class CORE_EX1_066:
    """Acidic Swamp Ooze - simplified vanilla minion.

    Original: Battlecry: Destroy your opponent's weapon.
    Weapons are disabled for now, so this is safe as vanilla.
    """
    pass


class TSC_908:
    """Sir Finley, Sea Guide - simplified vanilla minion.

    Original: Battlecry swaps your hand with the bottom of your deck.
    Simplified to avoid generated hand/deck/stat crash paths.
    """
    pass


class CORE_EX1_046:
    """Dark Iron Dwarf - simplified.

    Original: Battlecry: Give a minion +2 Attack this turn.
    Simplified to stable minion-target +2 Attack buff.
    """
    requirements = {
        PlayReq.REQ_TARGET_TO_PLAY: 0,
        PlayReq.REQ_MINION_TARGET: 0,
...
class CORE_LOE_076:
    """Sir Finley Mrrgglton - simplified vanilla minion.

    Original: Battlecry: Discover a new basic Hero Power.
    Simplified because generated choice/hero-power logic can crash is_playable.
    """
    pass

class CATA_556:
    """Carrier Whelp - simplified vanilla minion.

    Simplified because generated stat/aura logic can crash is_playable.
    """
    pass
...
class REV_240:
    ...
    Original effect involves hand/deck manipulation.
    Simplified because generated logic can crash is_playable.
    """
    pass


class LOE_110:
    """Ancient Shade - simplified vanilla minion.

    Original shuffles an Ancient Curse into your deck.
    Simplified because generated deck/stat logic can crash is_playable.
    """
    pass

class TLC_109:
    """Relic Miner - simplified no-op.

    Simplified because generated/random keyword logic can try to apply Poisonous
    to a Hero.
    """
    pass
...
class BOT_401:
    """Weaponized Piñata - simplified vanilla minion.

    Original Deathrattle adds a random Legendary minion to your hand.
    Simplified because random generation can create illegal keyword/hero paths.
    """
    pass

class CORE_ICC_900:
    """Necrotic Geist - simplified vanilla minion.

    Original: Whenever one of your other minions dies, summon a 2/2 Ghoul.
    Simplified because generated death/event logic can crash is_playable.
    """
    pass


class CORE_REV_839:
    """Sinstone Totem - simplified vanilla minion.

    Original has end-of-turn/buff behavior.
    Simplified because generated aura/stat logic can crash is_playable.
    """
    pass


class BAR_020:
    """Razormane Raider - simplified vanilla minion.
...
class CORE_CS2_117:
    """Earthen Ring Farseer - simplified safe heal.

    Original: Battlecry: Restore 3 Health.
    """
    requirements = {
        PlayReq.REQ_TARGET_TO_PLAY: 0,
    }
...
class VAC_430:
    """Bloodsail Recruiter - simplified vanilla minion.

    Simplified because generated effect can try to apply Lifesteal to a Hero.
    """
    pass
...
class CFM_321:
    ...
    Original: Discover a Hunter, Paladin, or Warrior card.
    Simplified because generated Discover can apply illegal Windfury to a Hero.
    """
    pass


class YOP_029:
    """Resizing Pouch - simplified no-op.

    Original: Discover a card with Cost equal to your remaining Mana Crystals.
    Simplified because generated Discover can apply illegal Lifesteal to a Hero.
    """
    pass
...
class CORE_GIL_622:
    """Lifedrinker - simplified safe battlecry.

    Original: Battlecry: Deal 3 damage to the enemy hero. Restore 3 Health to your hero.
    Simplified with direct stable hero damage/heal.
    """
    play = (
        Hit(ENEMY_HERO, 3),
...
class TSC_007:
    """Gangplank Diver - simplified vanilla minion.

    Simplified because generated dormant/immune/stat logic can crash is_playable.
    """
    pass


class RLK_569:
    """Potion Belt - simplified no-op.

    Original: Discover a Concoction.
    Simplified because generated Discover/random logic can apply Poisonous to a Hero.
    """
    pass


class SCH_270:
    """Primordial Studies - simplified no-op.

    Original: Discover a Spell Damage minion. Your next one costs (1) less.
    Simplified because generated Discover/cost/stat logic can crash is_playable.
    """
    pass


class CFM_852:
    """Lotus Agents - simplified no-op.

    Original: Discover a Druid, Rogue, or Shaman card.
    Simplified because generated Discover/random logic can apply Windfury to a Hero.
    """
    pass


class TLC_245:
    """Ancient Raptor - simplified vanilla minion.
...
class CORE_CS2_181:
    """Injured Blademaster - simplified vanilla minion.

    Original: Battlecry: Deal 4 damage to himself.
    Simplified because generated self-damage/stat logic can crash is_playable.
    """
    pass


class LOOT_154:
    """Gravelsnout Knight - simplified vanilla minion.

    Original: Battlecry: Summon a random 1-Cost minion for your opponent.
    Simplified because generated random summon/stat logic can crash is_playable.
    """
    pass

class WW_428:
    """Eroded Sediment - simplified no-op.

    Original has Discover/random generation behavior.
    Simplified because generated random logic can try to apply Poisonous to a Hero.
    """
    pass

class TIME_046:
    """Cyborg Patriarch - simplified vanilla minion.
...
class DMF_065:
    """Banana Vendor - simplified vanilla minion.

    Original: Battlecry adds Bananas to each player's hand.
    Simplified because generated give/stat logic can crash is_playable.
    """
    pass


class TTN_723:
    """Tram Operator - simplified vanilla minion.
...
class FP1_022:
    """Voidcaller - simplified vanilla minion.

    Original: Deathrattle summons a Demon from your hand.
    Simplified because generated summon/deathrattle logic can crash is_playable.
    """
    pass


class ONY_003:
    """Whelp Bonker - simplified vanilla minion.
...
class LOOT_291:
    """Shroom Brewer - simplified safe heal.

    Original: Battlecry: Restore 4 Health.
    """
    requirements = {
        PlayReq.REQ_TARGET_TO_PLAY: 0,
    }
...
class ULD_161:
    """Neferset Thrasher - simplified vanilla minion.

    Original damages your hero after attacking.
    Simplified because generated damage/stat logic can crash is_playable.
    """
    pass


class YOP_030:
    """Felfire Deadeye - simplified vanilla minion.

    Original reduces hero power cost.
    Simplified because generated cost/stat logic can crash is_playable.
    """
    pass

class BT_713:
    """Akama - simplified vanilla minion.

    Original has Stealth and shuffles Akama Prime into your deck.
    Simplified because generated stealth/deck/stat logic can crash is_playable.
    """
    pass


class CORE_GIL_598:
    """Tess Greymane - simplified vanilla minion.

    Original replays cards from other classes.
    Simplified because generated replay/random logic can crash is_playable.
    """
    pass

class DEEP_035:
    """Iridescent Gyreworm - simplified vanilla minion.
...
class BAR_027:
    """Darkspear Berserker - simplified vanilla minion.

    Original damages your hero.
    Simplified because generated damage/stat logic can crash is_playable.
    """
    pass
...
class LOOT_111:
    """Scorp-o-matic - simplified vanilla minion.

    Original destroys a minion with 1 or less Attack.
    Simplified because generated target/stat logic can crash is_playable.
    """
    pass

class NEW1_037:
    """Master Swordsmith - simplified vanilla minion.

    Original: At end of your turn, give another friendly minion +1 Attack.
    Simplified because generated aura/stat logic can crash is_playable.
    """
    pass
...
class BT_732:
    """Scavenging Shivarra - simplified vanilla minion.
...
class CATA_209:
    """Battlefield Blaster - simplified vanilla minion.
...
class CORE_MAW_002:
    """Habeas Corpses - simplified no-op.

    Original has corpse/discover/random behavior.
    Simplified because generated logic can apply Lifesteal to a Hero.
    """
    pass

class CORE_REV_900:
    """Scuttlebutt Ghoul - simplified vanilla minion.
...
class CORE_NEW1_020:
    """Wild Pyromancer - simplified vanilla minion.

    Original: After you cast a spell, deal 1 damage to all minions.
    Simplified because generated event/stat logic can crash is_playable.
    """
    pass


class GIFT_01:
    """Harth Stonebrew - simplified vanilla minion.

    Original has complex random hand replacement behavior.
    Simplified because generated random/stat logic can crash is_playable.
    """
    pass


class VAC_438:
    """Travel Agent - simplified no-op.

    Simplified because generated/random logic can try to apply Windfury to a Hero.
    """
    pass


class TRL_096:
    """Griftah - simplified no-op.
...
class EDR_888:
    """Malorne the Waywatcher - simplified vanilla minion.

    Simplified because generated/random logic can try to apply Lifesteal to a Hero.
    """
    pass

# ---------------------------------------------------------------------------
# Manual safe overrides: confirmed 5000-game is_playable failures
# ---------------------------------------------------------------------------

class YOP_012:
    """Deathwarden - simplified vanilla minion.

    Original affects Deathrattles.
    Simplified because generated aura/deathrattle logic can crash is_playable.
    """
    pass
...
class WC_029:
    """Selfless Sidekick - simplified vanilla minion.

    Original interacts with weapons/attacks.
    Simplified because generated stat/aura logic can crash is_playable.
    """
    pass
...
class SC_020:
    """Consume - simplified no-op.

    Original destroys a friendly minion and gives stats.
    Simplified because generated buff/destroy logic can crash is_playable.
    """
    pass


class AT_019:
    """Dreadsteed - simplified vanilla minion.

    Original repeatedly resummons itself.
    Simplified because generated deathrattle/summon recursion can crash is_playable.
    """
    pass


class GVG_085:
    """Annoy-o-Tron - simplified vanilla minion.

    Original has Taunt and Divine Shield.
    Simplified because generated keyword/stat logic can crash is_playable.
    """
    pass


class ICC_099:
    """Ticking Abomination - simplified vanilla minion.

    Original Deathrattle deals damage to your minions.
    Simplified because generated deathrattle/stat logic can crash is_playable.
    """
    pass
...
class TTN_083:
    """Son of Hodir - simplified vanilla minion.
...
class VAN_tt_004:
    """Flesheating Ghoul - simplified vanilla minion.

    Original gains Attack whenever a minion dies.
    Simplified because generated death/event stat logic can crash is_playable.
    """
    pass


class CFM_851:
    """Daring Reporter - simplified vanilla minion.

    Original gains +1/+1 whenever your opponent draws a card.
    Simplified because generated draw/event stat logic can crash is_playable.
    """
    pass

class SW_006:
    """Stubborn Suspect - simplified vanilla minion.
...
class WORK_042:
    """Carnivorous Cubicle - simplified vanilla minion.

    Simplified because generated transform/copy/deathrattle logic can crash is_playable.
    """
    pass


class BAR_026:
    """Death's Head Cultist - simplified vanilla minion.

    Original has Taunt and Deathrattle heal.
    Simplified because generated deathrattle/stat logic can crash is_playable.
    """
    pass
...
class CORE_CS2_122:
    """Raid Leader - simplified vanilla minion.

    Original gives your other minions +1 Attack.
    Simplified because aura stat logic can crash is_playable.
    """
    pass


class CFM_647:
    """Blowgill Sniper - simplified safe battlecry.

    Original: Battlecry: Deal 1 damage.
    """
    requirements = {
        PlayReq.REQ_TARGET_TO_PLAY: 0,
    }
...
class VAN_CS2_188:
    """Abusive Sergeant - simplified vanilla minion.

    Original temporarily gives a minion +2 Attack.
    Simplified because temporary stat buffs can crash is_playable.
    """
    pass


class VAN_CS2_213:
    """Reckless Rocketeer - simplified vanilla minion.
...
class LOE_110:
    ...
    Original shuffles Ancient Curse into your deck.
    Simplified because generated shuffle/deck logic can crash is_playable.
    """
    pass


class TLC_468:
    """Blob of Tar - simplified vanilla minion.
...
class AT_046:
    """Tuskarr Totemic - simplified vanilla minion.

    Original summons a random Totem.
    Simplified because random summon list logic can cause:
    AttributeError: 'list' object has no attribute 'eval'
    """
    pass

# ---------------------------------------------------------------------------
# Manual safe overrides: cost-reduction/random-cost cards
# ---------------------------------------------------------------------------

class AT_007:
    """Spellslinger - simplified vanilla minion.

    Original: Both players get a random spell. Yours costs (2) less.
    Simplified because random spell generation + cost enchantment logic can
    trigger KeyError paths during broad random simulation.
    """
    pass
...
class DRG_257:
    """Frizz Kindleroost - simplified vanilla minion.

    Original reduces the Cost of Dragons in your deck.
    Simplified because broad cost-reduction logic can trigger AT_007e-related
    crashes.
    """
    pass
...
class BRM_018:
    """Dragon Consort - simplified vanilla minion.

    Original reduces the Cost of the next Dragon you play.
    Simplified because generated cost-reduction logic can trigger AT_007e paths.
    """
    pass


class BT_026:
    """Aldor Truthseeker - simplified vanilla minion.

    Original discounts Librams.
    Simplified because generated cost-reduction logic can crash broad simulation.
    """
    pass


class ICC_837:
    """Bring It On! - simplified no-op.

    Original gives Armor and reduces enemy minion Costs.
    Simplified because generated cost-reduction logic can trigger AT_007e paths.
    """
    pass
...
class EX1_145:
    """Preparation - simplified no-op.

    Original reduces the Cost of your next spell.
    Simplified because generated cost-reduction logic can crash broad simulation.
    """
    pass
...
class AT_046:
    ...
    Original summons a random Totem.
    Simplified because generated random summon list logic can cause:
    AttributeError: 'list' object has no attribute 'eval'.
    """
    pass

class VAN_CS2_162:
    """Lord of the Arena - simplified vanilla minion."""
    pass


class CORE_LOOT_137:
    """Sleepy Dragon - simplified vanilla minion."""
    pass


class CORE_EX1_058:
    """Sunfury Protector - simplified vanilla minion."""
    pass

class RLK_029:
    """Shatterskin Gargoyle - simplified vanilla minion.
...
class EDR_486:
    """Scorching Observer - simplified vanilla minion.
...
class DRG_402:
    """Sathrovarr - simplified vanilla minion.

    Original copies a friendly minion to hand/deck/battlefield.
    Simplified because copy/summon/deck logic can crash is_playable.
    """
    pass
...
class OG_271:
    """Scaled Nightmare - simplified vanilla minion.

    Original doubles its Attack each turn.
    Simplified because generated event/stat logic can crash is_playable.
    """
    pass

def _sanitize_all_numeric_tags():
    """Final pass over all card classes to remove unsafe numeric tag values."""
    numeric_tags = {
        GameTag.ATK,
        GameTag.HEALTH,
        GameTag.COST,
    ...
    for name, cls in list(globals().items()):
        ...
        for tag, value in tags.items():
            if tag in numeric_tags and not isinstance(value, int):
                continue
            cleaned[tag] = value
