...
def make_game():
    class1 = PLAYER1_CLASS
    class2 = PLAYER2_CLASS

    deck1 = pad_deck(PLAYER1_DECK, class1)
    deck2 = pad_deck(PLAYER2_DECK, class2)

    p1 = Player("Deck1", deck1, HEROES[class1])
    p2 = Player("Deck2", deck2, HEROES[class2])
...
def main():
    ...
    print("\nDone.")
    print("\n===== WINRATE REPORT =====")
    total_finished = results["Deck1"] + results["Deck2"] + results["Draw"]

    if total_finished:
        print(f"Deck1 wins: {results['Deck1']} / {total_finished} = {results['Deck1'] / total_finished:.1%}")
        print(f"Deck2 wins: {results['Deck2']} / {total_finished} = {results['Deck2'] / total_finished:.1%}")
        print(f"Draws:      {results['Draw']} / {total_finished} = {results['Draw'] / total_finished:.1%}")
