...
class SwapAttackHealth(TargetedAction):
    """
    Swap the Attack and Health of minion targets.
    Used by cards like AT_016 / Confuse.
    """
    ...
    def do(self, source, target):
        ...
        # Keep current damage sane after changing max health.
        # If the new max health is lower than current health, clamp it.
        if target.health > target.max_health:
            target.damage = 0
        else:
            target.damage = max(0, target.max_health - target.health)
