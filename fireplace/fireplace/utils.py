...
CARD_SETS = sorted(
    cs
    for _, cs, ispkg in iter_modules([_cards_module])
    if not ispkg and cs not in ("__init__", "utils", "modern")
)

# Load modern last so existing real implementations win first.
CARD_SETS.append("modern")
T = TypeVar("T")
