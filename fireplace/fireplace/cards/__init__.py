...
class CardDB(dict[str, cardxml.CardXML]):
    ...
    def initialize(self, locale=default_language):
        ...
        carddefs_file = os.environ.get("FIREPLACE_CARDDEFS", "CardDefs.xml")
        filename = os.path.join(dirname, carddefs_file)
        
        log.info("Loading CardDefs from %s", filename)
        
        if not os.path.exists(filename):
            raise FileNotFoundError(f"CardDefs file not found: {filename}")
