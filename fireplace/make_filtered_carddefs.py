...
ALWAYS_EXCLUDE_IDS = {
    ...
    "CORE_EX1_246",
    "CFM_816",  # Virmen Sensei: owner/buffs crash
    "DS1_070",      # Houndmaster: owner/buffs crash
    "VAN_DS1_070",  # Houndmaster: owner/buffs crash
    "AT_046",   # Tuskarr Totemic: selector/list crash
    "CS2_022",  # Polymorph: transform crash
    "AT_118",  # Grand Crusader: random Paladin card can generate unsupported lifesteal-on-hero crash
    "VAN_CS2_022",   # Polymorph: transform crash
    "VAC_324",       # Matching Outfits: owner/buffs or unsupported buff crash
    "WC_017",        # Savory Deviate Delight: transform crash
    "ETC_362",       # JIVE, INSECT!: transform/evolve crash
    "TID_708",       # Polymorph: Jellyfish transform crash
    "WON_300",       # Virmen Sensei: owner/buffs crash
    "YOD_020",       # Explosive Evolution: transform/evolve crash
    "CORE_REV_945",  # Sketchy Stranger: random secret/discover instability
    "LOE_061",       # Anubisath Sentinel: deathrattle buff crash
    "EX1_246",       # Hex: transform crash
    "ETC_077",  # Disc Jockey: function/stat TypeError
    "DRG_270t6",  # Malygos's Polymorph: transform crash
    "GIL_116",    # Arcane Keysmith: discover/secret instability
    "BRM_022",    # Flamewaker: random spell damage buff crash
}
...
ALWAYS_KEEP_IDS = {
    ...
    "AT_007e",      # Spellslinger cost reduction enchantment
    "AT_054e",      # The Mistcaller enchantment
    "AT_057e",      # Stablemaster enchantment
    "BAR_313e",     # Generic +3/+3 enchantment used by generated scripts
}

def apply_excludes(keep_ids):
    # Remove only the exact excluded card IDs.
    # Do NOT remove enchantments/tokens like AT_054e just because AT_054 is excluded.
...
def main():
    ...
    for entity in all_entities:
        card_id = get_card_id(entity)
        if card_id in keep_ids and (card_id not in ALWAYS_EXCLUDE_IDS or card_id in ALWAYS_KEEP_IDS):
